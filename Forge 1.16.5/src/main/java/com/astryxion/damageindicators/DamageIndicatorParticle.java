...
public class DamageIndicatorParticle extends Particle {
    ...
    public void render(IVertexBuilder vertexConsumer, ActiveRenderInfo camera, float partialTicks) {
        ...
        double x = MathHelper.lerp((double) partialTicks, this.xo, this.x);
        double y = MathHelper.lerp((double) partialTicks, this.yo, this.y);
        double z = MathHelper.lerp((double) partialTicks, this.zo, this.z);
        ...
        IReorderingProcessor order = damageString.getVisualOrderText();
        float f = (float) (-Minecraft.getInstance().font.width(order) / 2);
        poseStack.scale(particleScale, particleScale, particleScale);
        poseStack.translate(0.0F, -2.0F, 0.0F);
        if (Config.INSTANCE.active().damageParticleOutline.get()) {
            DamageIndicators.drawInBatch8xOutline(Minecraft.getInstance().font, order,
                    poseStack.last().pose(), f, 0.0F, color, colorOutline, buffer, false, 15728880);
        } else {
            Minecraft.getInstance().font.drawInBatch(order, f, 0.0F, color, false, poseStack.last().pose(), buffer, false, 0, 15728880);
        }
        buffer.endBatch();
        poseStack.popPose();
    }
