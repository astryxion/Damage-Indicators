...
/**
 * World-space popoffs for Fabric 1.21.11.
 * <p>
 * Spawn: client tick health delta (mixin alone does not fire reliably here).
 * Render: FloatingDamageIndicators hook ({@link WorldRenderEvents#AFTER_ENTITIES} +
 * event PoseStack + {@code scale(s,-s,s)} billboard). Do NOT apply particle-style Z-180 on the
 * event stack — that combination makes text invisible.
 */
public final class PopoffRenderer {
    ...
    private static void onAfterEntities(net.fabricmc.fabric.api.client.rendering.v1.world.WorldRenderContext context) {
        if (ENTRIES.isEmpty()) {
            return;
        }
        Minecraft mc = Minecraft.getInstance();
    ...
    /** Ensure RGB colors from Forge have opaque alpha for 1.21.11 font rendering. */
