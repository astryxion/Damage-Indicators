 ...
 * Ported to NeoForge 1.21.11 GuiGraphics (Matrix3x2fStack + RenderPipelines).
 */
public final class Style2HudRenderer {
     ...
     */
    private static void blitFull(GuiGraphics guiGraphics, Identifier texture, int x, int y, int width, int height, int texW, int texH, int color) {
        guiGraphics.blit(RenderPipelines.GUI_TEXTURED, texture, x, y, 0.0F, 0.0F, width, height, texW, texH, texW, texH, color);
    }

    private static void blitRegion(GuiGraphics guiGraphics, Identifier texture, int x, int y, int width, int height,
    ...
    private static void drawHudText(GuiGraphics guiGraphics, Component text, int x, int y, int color, boolean outline) {
        Font font = Minecraft.getInstance().font;
        ...
        if (outline) {
            ...
            guiGraphics.drawString(font, text, x, y + 1, 0xFF000000, false);
            guiGraphics.drawString(font, text, x, y, argb, false);
        } else {
            guiGraphics.drawString(font, text, x, y, argb, true);
        }
    }

    private static void renderPortraitEntity(GuiGraphics guiGraphics, LivingEntity entity, int x0, int y0, int x1, int y1, float hudScale) {
        Config.StyleSettings cfg = Config.INSTANCE.active();
