...
/**
 * World-space popoffs for Fabric 26.2.
 * <p>
 * Spawn: client tick health delta (mixin alone does not fire reliably here).
 * Render: FloatingDamageIndicators hook ({@link LevelRenderEvents#COLLECT_SUBMITS} +
...
public final class PopoffRenderer {
    ...
    public static void init() {
        ...
        LevelRenderEvents.COLLECT_SUBMITS.register(PopoffRenderer::onCollectSubmits);
    }
    ...
    private static void onCollectSubmits(net.fabricmc.fabric.api.client.rendering.v1.level.LevelRenderContext context) {
        if (ENTRIES.isEmpty()) {
            return;
        }
        Minecraft mc = Minecraft.getInstance();
