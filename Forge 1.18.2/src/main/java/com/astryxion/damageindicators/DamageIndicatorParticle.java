...
public class DamageIndicatorParticle extends Particle {
    ...
    protected DamageIndicatorParticle(ClientLevel clientLevel, double x, double y, double z, double damageAmount, boolean heal) {
        ...
        this.damageString = new TextComponent(text);
        this.heal = heal;
        this.scale = 1.0F;
    ...
    public void render(VertexConsumer vertexConsumer, Camera camera, float partialTicks) {
        ...
        if (Config.INSTANCE.active().damageParticleOutline.get()) {
            ...
            Minecraft.getInstance().font.drawInBatch(damageString.getVisualOrderText(), f, 0.0F, color, false, posestack.last().pose(), multibuffersource$buffersource, false, 0, 15728880);
        }
